# Experiment: E-GDN-MQAR-BASELINE-007
...
- Approval: after a read-only helper status proved that GPU2 had eventually
  ...
  with unchanged science
- Target: logical AIStation `GPU2` only
- Approval-time GPU2 status: request
  `08a8c186-788c-42dc-b899-e4f645d05c61` was `Running`, reported exact
...
P005 passed locked setup, dependency checks, cache preparation, and a real A100
GDN/MQAR smoke. P006 then stopped before probe because the first Running status
temporarily exposed `resource=GPU:1`; a later read-only status reported the
same request as exact A100, but only after its one admission attempt was spent
and its lease had fallen below the initial floor. P007 tests one operational
hypothesis: wait for discovery status to expose the exact resource contract,
then consume the sole strict admission attempt on a completely fresh lease and
spend it immediately on the already-audited launcher.
...
1. No P007 allocation action or remote path may be created before the approved
   ledger is GitHub-verified. After verification, save one exact `status GPU2`
   response, bind the ledgered old request and unchanged image, then invoke
   exactly one `restart GPU2`. The restart response must bind a new non-empty
   request ID, the unchanged image, an allowed active state, and the exact
   `pause_requested` then `resume_requested` action pair. Any failure consumes
   the only restart. Pending or Queuing is wait-only. A Running response with
   generic `GPU:1` metadata is also status-only; it must not consume the sole
   admission attempt. Only a fresh saved status that simultaneously reports
   exact `NVIDIA-A100-SXM4-80GB:1` and at least 13,200 seconds may enter initial
   admission. GPU1 must never be queried, probed, opened, stopped, or mutated.
2. The frozen local admission module invokes the approved helper itself. For
   ...
   and after it. Any mismatch consumes the phase and terminates P007 before a
...
3. `/huyang2/zoology` must be a clean detached checkout of the exact formal
   ...
   duplicate stage consumes P007; a direct helper call may only poll read-only
...
5. Durable preflight-v2 then executes exactly `./setup.sh`, `./run.sh check`,
   ...
   chain. A failed/missing terminal or verifier consumes P007 and is not
   restarted.
...
may proceed. The restart is never re-entered.
    ...
    RUN_ID=gdn-mqar-single-baseline-p007-20260801t064410z
    ...
    LEDGERED_WORKSPACE=${P007_LEDGERED_WORKSPACE:?set from GitHub-verified allocation ledger}
...
Post-restart discovery uses only literal GPU2. Every response is saved under a
new name and never overwritten. Halt is terminal because the sole restart has
already been consumed. Pending, Queuing, ImagePulling, or Running with generic
`GPU:1` metadata permits only another uniquely named status poll. Discovery
never probes or mutates the source. Only exact A100 plus the full initial lease
floor may reach the initial admission capture below.

    DISCOVERY_RAW=$(next_numbered_path status-discovery) || exit 1
    guarded_node "$DISCOVERY_RAW" status GPU2
    read -r DISCOVERY_STATE DISCOVERY_WORKSPACE DISCOVERY_READINESS < <(
      $PYTHON - "$DISCOVERY_RAW" "$LEDGERED_WORKSPACE" <<'PY'
    import json
    import sys
    from pathlib import Path
    ...
    remaining = target.get("remainTime")
    if (
        target.get("wpName") != "GPU2"
        or state not in {"Halt", "Pending", "Queuing", "ImagePulling", "Running"}
        or (
            state != "Running"
        ...
        or not workspace
        or workspace != sys.argv[2]
    ):
        raise SystemExit(1)
    if state == "Running" and resource == "NVIDIA-A100-SXM4-80GB:1":
        if type(remaining) is not str or not remaining.isdigit():
            raise SystemExit(1)
        readiness = "ready" if int(remaining) >= 13200 else "insufficient"
    elif state == "Running" and resource == "GPU:1":
        readiness = "wait"
    elif state in {"Pending", "Queuing", "ImagePulling"}:
        readiness = "wait"
    else:
        readiness = "terminal"
    print(state, workspace, readiness)
    PY
    )

    # Waiting states end this controller turn without consuming admission.
    # An exact but short fresh lease is terminal; the sole restart is never
    # repeated. Only ready falls through to capture.
    case "$DISCOVERY_READINESS" in
      wait) exit 75 ;;
      ready) ;;
      insufficient|terminal) exit 1 ;;
      *) exit 1 ;;
    esac
...
any outer/inner failure consumes this stage and P007; it is not re-entered.
...
600 seconds respectively; exhausting a bound fails P007 rather than consuming
